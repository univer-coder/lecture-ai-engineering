...
def display_feedback_form():
    ...
    with st.form("feedback_form"):
        ...
        correct_answer = st.text_area("より正確な回答があれば教えてください！（任意）", key="correct_answer_input", height=100)
...
def display_history_list(history_df):
    ...
    for i, row in paginated_df.iterrows():
        with st.expander(f"{row['timestamp']} - Q: {row['question'][:50] if row['question'] else 'N/A'}..."):
            ...
            # NaNの場合はハイフン表示
            cols[3].metric("BLEU", f"{row['bleu_score']:.4f}" if pd.notna(row['bleu_score']) else "-")
            cols[4].metric("類似度", f"{row['similarity_score']:.4f}" if pd.notna(row['similarity_score']) else "-")
            cols[5].metric("関連性", f"{row['relevance_score']:.4f}" if pd.notna(row['relevance_score']) else "-")
...
def display_data_page():
    ...
    count = get_db_count()
    st.write(f"現在のデータベースには {count} 件のQ&Aがあります。")
